...
app.get('/api/documents', async (c) => {
    ...
    try {
        const { results } = await c.env.DB.prepare(`SELECT * FROM documents WHERE session_id = ? ORDER BY created_at DESC`).bind(session_id).all();
...
app.post('/api/documents', async (c) => {
    ...
    try {
        ...
        await c.env.DB.prepare(`INSERT INTO documents (id, session_id, filename, status, character_count, page_count, created_at)
       VALUES (?, ?, ?, 'processing', 0, 0, ?)`).bind(documentId, session_id, file.name, now).run();
        // 3. Process extraction asynchronously using c.executionCtx.waitUntil
        c.executionCtx.waitUntil(processExtraction(documentId, session_id, buffer, c.env));
...
app.delete('/api/documents/:id', async (c) => {
    ...
    try {
        ...
        const doc = await c.env.DB.prepare(`SELECT * FROM documents WHERE id = ? AND session_id = ?`).bind(documentId, session_id).first();
        if (!doc) {
        ...
        const { results: chunks } = await c.env.DB.prepare(`SELECT vectorize_id FROM chunks WHERE document_id = ?`).bind(documentId).all();
        const vectorizeIds = chunks.map(chunk => chunk.vectorize_id).filter(Boolean);
        ...
        await c.env.DB.prepare(`DELETE FROM chunks WHERE document_id = ?`).bind(documentId).run();
        // 6. Delete document record from D1
        await c.env.DB.prepare(`DELETE FROM documents WHERE id = ? AND session_id = ?`).bind(documentId, session_id).run();
...
app.delete('/api/documents', async (c) => {
    ...
    try {
        ...
        const { results: docs } = await c.env.DB.prepare(`SELECT id FROM documents WHERE session_id = ?`).bind(session_id).all();
        ...
        if (docIds.length > 0) {
            ...
            const { results: chunks } = await c.env.DB.prepare(`SELECT vectorize_id FROM chunks WHERE document_id IN (${placeholders})`).bind(...docIds).all();
            const vectorizeIds = chunks.map(chunk => chunk.vectorize_id).filter(Boolean);
            ...
            await c.env.DB.prepare(`DELETE FROM chunks WHERE document_id IN (${placeholders})`).bind(...docIds).run();
        }
        // 2. Delete all documents for this session in D1
        await c.env.DB.prepare(`DELETE FROM documents WHERE session_id = ?`).bind(session_id).run();
        return c.json({ success: true });
    }
    catch (error) {
...
app.get('/api/debug/search', async (c) => {
    ...
    try {
        ...
        const response = (await c.env.AI.run('@cf/baai/bge-m3', { text: [query] }));
        ...
        const { results: chunks } = await c.env.DB.prepare(`SELECT c.*, d.filename 
       FROM chunks c
       JOIN documents d ON c.document_id = d.id
       WHERE c.id IN (${placeholders})`).bind(...chunkIds).all();
...
app.get('/api/chat', async (c) => {
    ...
    try {
        const { results: messages } = await c.env.DB.prepare(`SELECT * FROM messages WHERE session_id = ? ORDER BY created_at ASC`).bind(session_id).all();
        ...
        if (allCitedIds.size > 0) {
            ...
            const { results: chunks } = await c.env.DB.prepare(`SELECT c.id, c.text, c.page_number, d.filename 
         FROM chunks c
         JOIN documents d ON c.document_id = d.id
         WHERE c.id IN (${placeholders})`).bind(...idsArray).all();
...
app.post('/api/chat', async (c) => {
    ...
    try {
        // 1. Generate query embedding using Workers AI bge-m3
        const embedResponse = (await c.env.AI.run('@cf/baai/bge-m3', { text: [question] }));
...
User Question: ${question}`;
        ...
        await c.env.DB.batch([
            c.env.DB.prepare(`INSERT INTO messages (id, session_id, role, content, cited_chunk_ids, created_at)
         VALUES (?, ?, 'user', ?, '[]', ?)`).bind(userMsgId, session_id, question, now),
            c.env.DB.prepare(`INSERT INTO messages (id, session_id, role, content, cited_chunk_ids, created_at)
         VALUES (?, ?, 'assistant', ?, ?, ?)`).bind(assistantMsgId, session_id, answerText, JSON.stringify(citedChunkIds ?? []), now + 1)
        ]);
        // 6. Return response
...
app.get('/api/evals/cases', async (c) => {
    try {
        const { results } = await c.env.DB.prepare(`SELECT * FROM eval_cases`).all();
        return c.json(results);
    }
    catch (error) {
...
app.put('/api/evals/cases/:id', async (c) => {
    ...
    try {
        await c.env.DB.prepare(`UPDATE eval_cases
       SET question = ?, expected_answer = ?
       WHERE id = ?`).bind(question, expected_answer, id).run();
        return c.json({ ok: true });
    }
    catch (error) {
...
app.get('/api/evals/runs', async (c) => {
    try {
        const { results } = await c.env.DB.prepare(`SELECT * FROM eval_runs ORDER BY created_at DESC`).all();
        return c.json(results);
    }
    catch (error) {
...
app.post('/api/evals/run', async (c) => {
    try {
        ...
        await c.env.DB.prepare(`INSERT INTO eval_runs (id, created_at, total, correct, faithfulness_score)
       VALUES (?, ?, 0, 0, 0.0)`).bind(runId, now).run();
        // Reset case results in DB to clear out any previous run's scores
        await c.env.DB.prepare(`UPDATE eval_cases
       SET last_result = NULL, last_score = NULL`).run();
        return c.json({ runId });
    }
    catch (error) {
...
app.post('/api/evals/run-case/:id', async (c) => {
    ...
    try {
        ...
        const evalCase = await c.env.DB.prepare(`SELECT * FROM eval_cases WHERE id = ?`).bind(id).first();
        if (!evalCase) {
...
Do not include any other text, markdown formatting, or wrappers. Output raw JSON only.`;
        ...
        try {
            ...
            const isRateLimit = !isDailyQuota && (errMsg.includes('429') ||
                errMsg.toLowerCase().includes('rate limit') ||
                errMsg.includes('Max retries exceeded'));
            const isOverload = !isDailyQuota && !isRateLimit && (errMsg.includes('503') ||
                errMsg.includes('500'));
            judgeResult = {
                correct: false,
                faithfulness: 0,
       ...
       WHERE id = ?`).bind(JSON.stringify(lastResultObj), judgeResult.faithfulness, id).run();
        // 6. Return response
        return c.json({
            id,
            question,
        ...
        if (isDailyQuota) {
           ...
           WHERE id = ?`).bind(JSON.stringify(lastResultObj), id).run();
            }
            catch (dbErr) {
...
app.post('/api/evals/complete-run', async (c) => {
    ...
    try {
        ...
        const { results: cases } = await c.env.DB.prepare(`SELECT * FROM eval_cases`).all();
        let total = 0;
        let correct = 0;
        ...
        await c.env.DB.prepare(`UPDATE eval_runs
       SET total = ?, correct = ?, faithfulness_score = ?
       WHERE id = ?`).bind(total, correct, avgFaithfulness, runId).run();
        // 3. Fetch and return the updated run
        const updatedRun = await c.env.DB.prepare(`SELECT * FROM eval_runs WHERE id = ?`).bind(runId).first();
...
async function processExtraction(documentId, session_id, buffer, env) {
    try {
        ...
        if (totalChars < 50) {
            await env.DB.prepare(`UPDATE documents 
         SET status = 'failed', error_message = ?
         WHERE id = ?`).bind('Scanned or empty PDF (no text found)', documentId).run();
            return;
        }
        ...
        for (let i = 0; i < chunkTexts.length; i += batchSize) {
            ...
            if (!response || !Array.isArray(response.data) || response.data.length !== batch.length) {
                throw new Error(`Embedding model response shape mismatch. Expected array of length ${batch.length}, got ${response?.data ? (Array.isArray(response.data) ? `array of length ${response.data.length}` : typeof response.data) : 'undefined'}`);
            }
        ...
        chunks.forEach((chunk, idx) => {
            ...
            // Add to D1 statement
            d1Statements.push(env.DB.prepare(`INSERT INTO chunks (id, document_id, chunk_index, text, page_number, vectorize_id)
           VALUES (?, ?, ?, ?, ?, ?)`).bind(chunkId, documentId, chunk.chunk_index, chunk.text, chunk.page_number, chunkId));
        });
       ...
       WHERE id = ?`).bind(language, totalPages, totalChars, documentId).run();
    }
    catch (error) {
        ...
        await env.DB.prepare(`UPDATE documents 
       SET status = 'failed', error_message = ?
       WHERE id = ?`).bind(errMsg, documentId).run();
    }
}
// Helper: HTTP Fetch with retry & exponential backoff on 429, 500, 503
async function fetchWithRetry(url, options, modelLabel, retries = 3, initialDelay = 2000) {
...
function extractCitedChunkIds(answer, chunks) {
